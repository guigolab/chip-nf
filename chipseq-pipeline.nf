...
.fromPath(params.input).map {
    matcher = fastq.name =~ fastqPattern
    if (!matcher) {
...
process mapping {
  ...
  command += "gt.filter -i ${out_prefix}.map.gz --max-levenshtein-error ${params.mismatches} -t ${cpus}| gt.filter --max-maps ${params.multimaps} -t ${cpus} | pigz -p ${cpus} -c > ${out_prefix}.filter.map.gz\n"
